    ...
    if is_CoSG==False:
        with open(protocol_file, 'r') as f:
            for line in f:
                ...
                utt_id = parts[1]
                label = parts[-1]
                labels[utt_id] = 1 if label == 'bonafide' else 0
    else:       
        with open(protocol_file, 'r') as f:
            for line in f:
                parts = line.strip().split()
                ...
                codec=parts[0]
                label = parts[-1]
                labels[f"{codec}/{utt_id}"] = 1 if label == 'bonafide' else 0
...
def compute_eval_eer(score_file, protocol_file, is_CoSG=False):
    ...
    target_scores = y_scores[y_true == 1]
    nontarget_scores = y_scores[y_true == 0]
...
def compute_f1_score(score_file, protocol_file, is_CoSG ,task=""):
    scores = {}
    labels = {}
    ...
    with open(protocol_file, 'r') as f:
        for line in f:
            ...
            if is_CoSG:
                ...
                codec=utt_id[9:]
                if task=="AS":
                    map_dict=AS_dict_CoRS
                elif task=="DS":
                    map_dict=DS_dict_CoRS
                elif task=="QS":
                    map_dict=QS_dict_CORS
                label = parts[-1]
...
def compute_multi_accuracy(score_file, protocol_file,type):
    scores = []
    utt_ids = []
    with open(score_file, 'r') as f:
        for line in f:
        ...
        for line in f:
            ...
            if len(parts) >= 5:
                ...
                if type=="AS":
                    ...
                    raise ValueError("type must be in [AS,DS,QS]")
