...
def make_positions_old(tensor, padding_idx, left_pad):
...
def make_positions(tensor, padding_idx, left_pad):
    ...
    # 直接創建新的位置張量，不使用緩衝區和out參數
    positions = torch.arange(padding_idx + 1, max_pos, device=tensor.device)
    ...
    new_tensor = tensor.clone()
    return new_tensor.masked_scatter_(mask, positions[mask].to(new_tensor.dtype)).long()
